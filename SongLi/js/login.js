$(document).ready(function () {
  ...
  if (logState) {
    $('.navbtnSP').append(
      `<button id="logOut" class="navbtn font_T_Bolder mx-2">登出</button>`
    );
    loginBtn = $('#loginRegBtn').detach();
    $('.userName').text(logState);
    $('a.btn.font_T_Bolder').attr('href', './cart.html');
    if ($("li.nav-item a[href='javascript:void(0)']").text()) {
      $("li.nav-item a[href='javascript:void(0)']").attr(
        'href',
        './member.html'
      );
    }
    $("a.btn.font_T_Bolder,li.nav-item a[href='./member.html']").off('click');
  } else {
    $('a.btn.font_T_Bolder').attr('href', 'javascript:void(0)');
    $('#logOut').remove();
    $('.userName').text('');
    // $('a.btn.font_T_Bolder').attr('href', 'javascript:void(0)')
    if ($("li.nav-item a[href='./member.html']").text()) {
      $("li.nav-item a[href='./member.html']").attr(
        'href',
        'javascript:void(0)'
      );
    }
  ...
  $('.navbtnSP').on('click', '#logOut', function () {
    ...
    if (window.location.pathname == url + '/html/member.html') {
      ...
      return false;
    } else if (window.location.pathname == url + '/html/about.html') {
      addForm();
    } else if (window.location.pathname == url + '/html/product.html') {
      $('#productList').off('click', 'div>div>.add-to-cart');
      $('#productList').on('click', 'div>div>.add-to-cart', function () {
        alert('請先登入');
      });
    } else if (window.location.pathname == url + '/html/product_info.html') {
      $('.add-to-cart').off('click');
      $('.add-to-cart').on('click', function () {
        alert('請先登入');
      });
    } else if (window.location.pathname == url + '/html/cart.html') {
      window.location.replace(
        window.location.protocol +
          '//' +
          window.location.hostname +
  ...
  $('#registerForm').on('submit', function (event) {
    ...
    if ($("input[name='uGender']:checked").val()) {
      ...
      if (
        ...
        genderCheck.innerText == '' &&
        uPwdCorrectText.innerText == '密碼一致' &&
        $("input[name='uChoice']:checked").val()
      ) {
        Register(formData);
        mail(formData);
        ...
        $('#mailText').empty();
        $('#mailText').append(
          `<p>親愛的 ${$('#uName').val()} 會員，請至註冊信箱收取驗證信</p>`
        );
      }
    }
  });
  function mail(formData) {
    $.ajax({
      url: '../php/phpmailer.php/register',
      method: 'POST',
  ...
  function Register(formData) {
    $.ajax({
      url: '../php/restful_userInfo.php/insert',
      method: 'POST',
      data: JSON.stringify(formData),
      contentType: 'application/json',
    });
  ...
  function check() {
    $.ajax({
      url: '../php/restful_userInfo.php/check',
      method: 'POST',
      contentType: 'application/json',
      data: JSON.stringify({
        uAccount: $('#uAccount').val(),
  ...
  function login() {
    $.ajax({
      url: '../php/restful_userInfo.php/login',
      method: 'POST',
      contentType: 'application/json',
      data: JSON.stringify({
      ...
      success: function (data) {
        ...
        if (data.status == 'OK') {
          ...
          if (logState) {
            ...
            loginBtn = $('#loginRegBtn').detach();
            $('a.btn.font_T_Bolder').attr('href', './cart.html');
            if (window.location.pathname == url + '/html/about.html') {
              addForm();
              hi(logState);
            } else if (window.location.pathname == url + '/html/product.html') {
              $('#productList').off('click', 'div>div>.add-to-cart');
            ...
            $("a.btn.font_T_Bolder,li.nav-item a[href='./member.html']").off(
              'click'
            );
          }
        }
  ...
  function forgetPwdCheck(search) {
    $.ajax({
      url: '../php/phpmailer.php/PWDforgot',
      method: 'post',
      contentType: 'application/json',
